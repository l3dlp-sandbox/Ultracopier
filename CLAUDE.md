...
- **Use ASan AND TSan as widely as possible — Ultracopier is heavily multi-threaded, and bugs are often THREAD-related.** Every copy runs Qt event loops + ListThread + N TransferThreads each with reader/writer threads, coordinated by semaphores, mutexes and cross-thread queued signals — data races, lost wakeups and ordering bugs are a first-class defect category here, and neither valgrind's memcheck nor ASan sees them. So when a change touches (or could interact with) anything threaded — the read/write pump, semaphore/mutex handshakes, signal/slot flow across threads, stop/pause/skip paths, the pipelined schedulers — run the affected case(s) under **ThreadSanitizer** (`-fsanitize=thread`, a SEPARATE build: TSan and ASan cannot be combined in one binary) in addition to the ASan lane. Prefer both lanes on every non-trivial change; drop one only when it genuinely cannot apply (e.g. TSan on a single-threaded utility). Treat a TSan race report that touches our code like a real bug — races are exactly the "works on my machine, corrupts under load" class (see the load-immunity directive).
